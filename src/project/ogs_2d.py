...
from project import get_srf, mesh_2d
...
#    model.msh.generate("radial", dim=2, rad=rad, angles=angles)
    model = mesh_2d.create_mesh(model)
    
#    model.gli.generate("radial", dim=2, rad_out=rad[-1], angles=angles)  
    model.gli.read_file('project/model.gli')
    
    return model

def write_ogs_project(model, owell_pos, transmissivity, 
                      pwell_pos = [[0, 0]], time=None):
    ...
    if model.task_root=="pump_2d_steady":
        model = add_steady_files(model, owell_pos, transmissivity, pwell_pos)
    elif model.task_root=="pump_2d_steady_multi":
        model = add_steady_files(model, owell_pos, transmissivity, pwell_pos)    
    else:
        model = add_trans_files(model, owell_pos, transmissivity, pwell_pos, time)
    
    return model


def add_steady_files(model, owell_pos, transmissivity, pwell_pos):
#    print(pwell_pos)
    model.gli.add_points([pwell_pos[0][0], pwell_pos[0][1], 0.0], "pwell")
    for i in range(len(owell_pos)):
        model.gli.add_points([owell_pos[i][0], owell_pos[i][1], 0.0], 
                             "owell_" + str(i))
...
def add_trans_files(model, owell_pos, transmissivity, pwell_pos, time):
    
    
    owell_no = owell_pos.shape[0]
